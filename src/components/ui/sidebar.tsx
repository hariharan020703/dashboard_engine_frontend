...
function Sidebar({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "relative h-svh w-(--sidebar-width) bg-transparent transition-[width] ease-linear duration-200",
      ...
      <div
        className={cn(
          "fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] ease-linear duration-200 md:flex",
          side === "left"
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon-sm"
      className={cn("size-8 text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground", className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
      aria-label={state === "collapsed" ? "Open sidebar" : "Minimize sidebar"}
      {...props}
    >
